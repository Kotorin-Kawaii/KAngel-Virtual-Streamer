...
from utils.streamer_prompt_generator import (
    persona_decision_prompt_builder,
    persona_qa_selector,
    streamer_reply_prompt_builder,
)
from utils.logger import logger
from core.persona_impact_analyzer import persona_impact_analyzer
from core.persona_dynamics import DynamicsContext, persona_dynamics
from core.internal_state import internal_state_dynamics
from core.audience_relationship import audience_relationship_manager
from core.event_bus import (
    PersonaMutation,
    SemanticImpactAnalyzedEvent,
    persona_event_pipeline,
)
from core.danmaku_memory import danmaku_memory_manager
from core.database_manager import db_manager
from core.danmaku_pool import danmaku_pool
from core.emotion_manager import emotion_manager
from core.nickname_history import nickname_history_context_manager
from core.long_term_memory import ConversationContinuityAnalyzer, long_term_memory_manager
...
class PersonaEngine:
    ...
    def __init__(self):
        ...
        # 尝试从数据库读取最新的人格状态
        latest_state = db_manager.get_latest_persona_state()
        if latest_state:
            # 使用数据库中的状态
        ...
        latest_internal_state = db_manager.get_latest_internal_persona_state()
        self.internal_state = (
        ...
        emotion_manager.restore_history(db_manager.get_recent_reply_emotions(limit=24))
    ...
    def update_state(self, delta: EmotionDelta, dynamics_context: Optional[DynamicsContext] = None):
        ...
        # 保存到数据库
        db_manager.save_persona_state(self.state.mood, self.state.stress, self.state.darkness)
    ...
    def _save_internal_state(self) -> None:
        db_manager.save_internal_persona_state(**self.internal_state.model_dump())
